    ...
    connect(this->ui->pushButton,&QPushButton::clicked,[=](){
        ...
        else
        {
            ...
            connect(this->Musiclist,&QListWidget::itemClicked,[=](QListWidgetItem *item)mutable{
                ...
                if(selectmusic("C:\\Users\\123\\Desktop\\FindMusic\\"+name+".mp3"))//使用自定义查找函数查找本地音乐缓存是否存在
                {
...
{
    QDir dir("C:/Users/123/Desktop/FindMusic");
    QStringList nameFilters=dir.entryList(QDir::Files);
    foreach(QString name,nameFilters)
    {
        qDebug()<<"select music this key is:"<<path;
        qDebug()<<"select music find key is:"<<"C:\\Users\\123\\Desktop\\FindMusic\\"+name;
        if(path=="C:\\Users\\123\\Desktop\\FindMusic\\"+name)
        {
            return true;
            break;
