...
{ 
    QString nowfile;
    while (1) {
        QFile fileIn("C:/Users/123/Desktop/Find.txt");
        if (fileIn.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            ...
            {          
                ...
                db.setPort(3306);
                db.setDatabaseName("user"); //不是Database,记得别填错，应该是连接名字
                db.setUserName("root");
                ...
                if (ok){
                    ...
                    QSqlQuery query(db);
                    QString sql=QString("select findlist_name,findlist_musicid from find_list where user_name='%1'").arg("finish");
                    qDebug()<<sql;
